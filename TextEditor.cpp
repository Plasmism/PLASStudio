...
void TextEditor::HandleKeyboardInputs()
{
    ...
    if (ImGui::IsWindowFocused())
    {
		...
		// ── text input (UTF‑8 queue) ────────────────────────────────
		if (!IsReadOnly() && !io.InputQueueCharacters.empty())
		{
			for (int i = 0; i < io.InputQueueCharacters.Size; ++i)
			{
				ImWchar c = io.InputQueueCharacters[i];
				// skip control chars we already process manually
				if (c >= 32)                     // 32 == space ─ allow printable ASCII+
					EnterCharacter(c, shift);    // feed into editor
			}
...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
{
	...
	if (aChar == '\n')
	{
		...
		if (mLanguageDefinition.mAutoIndentation) {
			// smart indent
			size_t i = 0;
			while (i < line.size() && (line[i].mChar == ' ' || line[i].mChar == '\t'))
				newLine.push_back(line[i++]);

			bool endsWithOpenBrace = false;
			for (size_t j = line.size(); j > 0; --j) {
				if (isspace(line[j - 1].mChar)) continue;
				if (line[j - 1].mChar == '{') endsWithOpenBrace = true;
				break;
			}
